...
FileNames.Destination_SampleMod_Merged_top910 = append(working_dir, 'Metagenome_GEM/models/SampleMod_Merged_top910.mat');
FileNames.Destination_SampleMod_Merged_top920 = append(working_dir, 'Metagenome_GEM/models/SampleMod_Merged_top920.mat');
FileNames.Destination_SampleMod_Merged_top930 = append(working_dir, 'Metagenome_GEM/models/SampleMod_Merged_top930.mat');
FileNames.Destination_SampleMod_Merged_top940 = append(working_dir, 'Metagenome_GEM/models/SampleMod_Merged_top940.mat');
FileNames.Destination_SampleMod_Merged_top950 = append(working_dir, 'Metagenome_GEM/models/SampleMod_Merged_top950.mat');
...
for i = 1:length(Transects);
    ...
    FileNames.Destination_PAgenes_Mod = append(working_dir, 'Metagenome_GEM/models/',Transect,'_PAgenes_Mod','.mat');
    ...
    %% Use Presence/Abs Matrix (i.e. CoreKOs are not forced and includes all KOs including those outside PanGEM KOs)
    ...
    % Precalculated and saves as 'PanGEM/data/models/essentialRxns.mat' so
    % commented out
    ...
    for a = 1:nSample
        for b = 1:nAlpha
            ...
            PAgenes_Mod(b).PresenceAbsenceMatrix = PAMat_model(:,:,b);
        end
    end
    ...
    Meta_Assembly_L3.PAMat_rxns_top910 = PAMat_rxns(:,:,4);
    Meta_Assembly_L3.PAMat_rxns_top920 = PAMat_rxns(:,:,5);
    Meta_Assembly_L3.PAMat_rxns_top930 = PAMat_rxns(:,:,6);
    Meta_Assembly_L3.PAMat_rxns_top940 = PAMat_rxns(:,:,7);
    Meta_Assembly_L3.PAMat_rxns_top950 = PAMat_rxns(:,:,8);
    %% Save metagenome sample models, PresAbs Model Table and L3 structure
    ...
    save(FileNames.Destination_PAgenes_Mod,'PAgenes_Mod');
...
SampleMod_Merged_top910 = SampleMod_Merged(4);
SampleMod_Merged_top920 = SampleMod_Merged(5);
SampleMod_Merged_top930 = SampleMod_Merged(6);
SampleMod_Merged_top940 = SampleMod_Merged(7);
SampleMod_Merged_top950 = SampleMod_Merged(8);
...
save(FileNames.Destination_SampleMod_Merged_top910,'SampleMod_Merged_top910');
save(FileNames.Destination_SampleMod_Merged_top920,'SampleMod_Merged_top920');
save(FileNames.Destination_SampleMod_Merged_top930,'SampleMod_Merged_top930');
save(FileNames.Destination_SampleMod_Merged_top940,'SampleMod_Merged_top940');
save(FileNames.Destination_SampleMod_Merged_top950,'SampleMod_Merged_top950');
...
%% Optional: Check that all the Core PanGEM KOs are included for top 930
% Checked that all the GEMs from metagenomes include core KOs.
Core_KO_PanGEM = readtable("~/Desktop/Project/BioGOSHIP_MetaPanGEM/Gene_match_PanGEM/Core_KO_PanGEM_curated.txt");
load(FileNames.L2Assembly_path)
load(FileNames.Destination_Samplelist_Merged)
nSample = numel(sampleList);
for a = 1:nSample;
    targSample = sampleList{a};
    geneSubset = SampleMod_Merged_top930.(targSample).geneSubset;
    ismember(Core_KO_PanGEM.KO,geneSubset);
    if sum(ismember(Core_KO_PanGEM.KO,geneSubset)) == numel(Core_KO_PanGEM.KO);
        disp(append('all core KOs included for ', targSample))
    else
        disp(append('not all core KOs included for ', targSample))
    end
end
